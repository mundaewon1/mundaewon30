...
- 문의 등록 및 조회
- 관리자 문의 관리 및 답변 등록
- 문의 상태에 따른 수정 제한 및 답변 처리 로직
- @TransactionalEventListener와 @Async를 활용한 비동기 답변 알림
- Local Filter + OpenAI API 기반 Hybrid 비속어 필터링
- 비속어 검출 결과에 따른 PENDING_REVIEW 상태 제어
- 관리자 페이지

| 답변 알림 이벤트 | 관리자 페이지 | AI 비속어 필터 |
| :---: | :---: | :---: |
| <img src="./img/qna-notification.png" width="250"> | <img src="./img/qna-admin.png" width="250"> | <img src="./img/qna-ai-filter.png" width="250"> |
...
- 단순 키워드 필터링만으로는 문맥에 따른 비속어·비방 표현을 정확하게 판단하기 어려웠으며, 모든 문의를 OpenAI API로 분석할 경우 API 비용 및 응답 지연이 발생

#### 해결

- Local Filter로 1차 필터링 후, 판단이 필요한 문의만 OpenAI API로 분석하는 Hybrid 필터링 방식 적용

#### 결과

- 불필요한 AI API 호출을 줄여 비용 및 응답 지연을 개선하고, 문맥을 고려한 비속어 필터링 구현

---

## 💡 프로젝트 소감
- OpenAI API와 Local Filter를 결합한 비속어 필터링 로직을 구현하며 AI 기능을 실제 서비스에 적용하는 방법을 경험
- Spring Event와 @Async를 활용한 비동기 처리로 답변 알림 기능을 구현하며 이벤트 기반 처리 구조를 이해
- 단순 기능 구현을 넘어 API 호출 비용과 응답 성능을 고려한 서비스 로직 설계의 중요성을 경험
