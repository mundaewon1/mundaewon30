...
## 🔧 트러블슈팅
- AI 기반 비속어 필터링 처리 개선
- 문제: 단순 키워드 필터링만으로는 문맥에 따른 비속어·비방 표현을 정확하게 판단하기 어려웠으며, 모든 문의를 OpenAI API로 분석할 경우 API 비용 및 응답 지연이 발생
- 해결: Local Filter로 1차 필터링 후, 판단이 필요한 문의만 OpenAI API로 분석하는 Hybrid 필터링 방식 적용
- 결과: 불필요한 AI API 호출을 줄여 비용 및 응답 지연을 개선하고, 문맥을 고려한 비속어 필터링 구현
