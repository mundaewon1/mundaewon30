...
function ReviewWritePage() {
    ...
    const queryMeetupId = router.isReady
        ? router.query.meetupId || router.query.id || router.query.meetup_id
        : null;

    const fromMypage = router.isReady ? router.query.from === "mypage" : false;
    const queryNotificationId = router.isReady
        ...
        const checkMeetupStatus = async () => {
            try {
                ...
                const response = await axios.get(
                    `${API_BASE_URL}/api/meetups/${targetMeetupId}`,
                    {
                        headers: {
            ...
            if (reviewDetail.images && Array.isArray(reviewDetail.images)) {
                const initialFiles = reviewDetail.images.map((img, idx) => {
                    ...
                    return {
                        uid: `-${idx}`,
                        name:
                            rawUrl.substring(rawUrl.lastIndexOf("_") + 1) ||
                            `image-${idx}.png`,
                        status: "done",
        ...
        const handleSuccessActions = async () => {
            if (success) {
                ...
                if (queryNotificationId) {
                    try {
                        await axios.patch(
                            `${API_BASE_URL}/api/notifications/reviews/${queryNotificationId}/read`,
                        );
                    } catch (err) {
                ...
                if (fromMypage) {
                    ...
                    const targetMeetupId =
                        queryMeetupId || reviewDetail?.meetupId;
                    router.push(
                        `/user/meetup/detail?meetupId=${targetMeetupId}&tab=review#review-section`,
                    );
                }
            }
        };
    ...
    const handleSubmit = async () => {
        ...
        try {
            const newFiles = fileList.filter(
                (file) => file.originFileObj && !file.id,
            );
            ...
            if (newFiles.length > 0) {
                ...
                const imageResponse = await axios.post(
                    `${API_BASE_URL}/api/reviews/images`,
                    formData,
                    {
                        headers: {
            ...
            const resolvedMeetupId = rawMeetupId
                ? Number(rawMeetupId)
                : reviewDetail && reviewDetail.meetupId
                  ? Number(reviewDetail.meetupId)
                  : undefined;
