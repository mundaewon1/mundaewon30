...
function questionDetail() {
    ...
    useEffect(() => {
        ...
        dispatch(fetchMeetupDetailRequest(Number(qna.parentId)));
    ...
    const canAnswer =
        user?.memberTypeId === 3 ||
        user?.memberTypeId === 4 ||
        (user?.memberTypeId === 1 && meetup?.memberId === user?.memberId);
    ...
    const statusColor = status === "ANSWERED" ? "green" : "orange";

    const publicText = qna?.isPublic === "N" ? "비공개 🔒" : "공개";
    ...
    // 문의 작성자 여부
    const isQuestionWriter = Number(user?.memberId) === Number(qna?.memberId);

    // 관리자 여부
    const isAdmin =
        Number(user?.memberTypeId) === 3 || Number(user?.memberTypeId) === 4;

    // 관리자 문의에만 만족도 기능 사용
    const canUseSatisfaction = !isMeetup;

    // 만족도 등록/수정 - 관리자 문의의 문의 작성자만 가능
    const canEvaluateSatisfaction = canUseSatisfaction && isQuestionWriter;

    // 만족도 삭제 - 문의 작성자 또는 관리자 가능
    const canDeleteSatisfaction =
        canUseSatisfaction && (isQuestionWriter || isAdmin);
    ...
    const handleSatisfaction = () => {
        ...
        if (answer.rating) {
            ...
            if (!window.confirm("만족도를 평가하시겠습니까?")) return;
        }
        dispatch(
            qnaSatisfactionRequest({
                answerId: Number(answer.answerId),
    ...
    const handleSatisfactionDelete = () => {
        if (!answer.answerId) return;
        if (!window.confirm("정말 만족도 평가를 삭제하시겠습니까?")) {
            return;
        }

        dispatch(qnaSatisfactionDeleteRequest(Number(answer.answerId)));
    };
    ...
    return (
        <div className="qna-write-page">
            <div className="qna-write-header">
                ...
                <Text type="secondary">문의 내용을 확인할 수 있습니다.</Text>
            </div>
            ...
            <Card className="qna-write-card">
                ...
                <Card size="small" className="qna-detail-content">
                    <Paragraph
                        style={{
                            whiteSpace: "pre-wrap",
                            marginBottom: 0,
        ...
        ================================================= */}
            ...
            <Card className="qna-write-card" style={{ marginTop: 16 }}>
                <Title level={4}>답변 정보</Title>
            ...
            {isMeetup && (
                ...
                >
                    <Button
                        onClick={() => router.push(`/user/mypage/question`)}
                    >
                        목록으로
                    </Button>
                </div>
