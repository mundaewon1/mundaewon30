...
function UserMyReviewPage() {
    ...
    const fetchNotifications = async () => {
        ...
        try {
            ...
            const response = await axios.get(
                `${API_BASE_URL}/api/notifications/reviews`,
                {
                    headers: {
    ...
    const handleNotificationClick = async (notificationId, meetupId) => {
        try {
            ...
            await axios.patch(
                `${API_BASE_URL}/api/notifications/reviews/${notificationId}/read`,
                null,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
            ...
            if (isMounted) {
                router.push(
                    `/user/meetup/review/write?meetupId=${meetupId}&from=mypage&notificationId=${notificationId}`,
                );
            }
        }
    };
    ...
    const columns = [
        {
            ...
            render: (meetupId, record) => (
                <div>
                    <Text strong>
                        {record.meetupTitle || `모임 #${meetupId}`}
                    </Text>
                    <br />
                ...
                return (
                    <Space size={6}>
                        <Button
                            ...
                            icon={<EditOutlined />}
                            onClick={() =>
                                handleEdit(currentReviewId, currentMeetupId)
                            }
                        >
                            수정
                        </Button>
                ...
                <Card
                    title={
                        <Space>
                            ...
                            <span>
                                리뷰 작성 권장 알림 ({notifications.length}개)
                            </span>
                        </Space>
                    }
                    style={{
                            ...
                            >
                                <List.Item.Meta
                                    ...
                                    description={
                                        <Text type="secondary">
                                            {item.content ||
                                                "즐거운 모임 되셨나요? 다른 회원님들을 위해 후기를 남겨주세요!"}
                                        </Text>
                                    }
                                />
                            </List.Item>
